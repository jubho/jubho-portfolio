$(document).ready(function(){
	...
	$(window).scroll(function(){
		...
		if( st>= aboutTop && st<port1Top){
			...
			$("#photo1 progress").animate({value : 75});
			$("#html progress").delay(100).animate({value : 70});
			$("#html1 progress").delay(100).animate({value : 70});
	...
	$("#popup").click(function(){
	   ...
	   $(document).ready(function() {
	   /////1. 퀵배너의 y좌표 (상대위치) 알아내기
	   const x = $("#quick").position().top;
	   console.log(x);
	   
	   //스크롤이벤트가 발생되었을때
	   $(document).scroll(function() {
	         //스크롤바를 얼마정도 내렸는지? 
			 const y = $(document).scrollTop(); //y좌표 
			 console.log(y);
			 
			 //스크롤바가 지정 위치를 찾아서 위아래로 움직인다 
			 $("#quick").stop().animate({top: x+y }, 250);
	   });
